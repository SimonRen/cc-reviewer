...
- `adapters/base.ts` - Adapter interface + registry for AI reviewers. Expert roles (security_auditor, performance_engineer, architect, correctness_analyst) provide specialized prompts
- `adapters/codex.ts`, `adapters/gemini.ts` - CLI-specific implementations
- `pipeline.ts` - Finding verification pipeline. FileCache for performance. Path traversal protection with `resolve()` + `normalize()`
- `schema.ts` - Zod schemas for structured output (ReviewFinding, Agreement, Disagreement, etc.)
...
- `consensus.ts` - Multi-model consensus building for council reviews
...
Optional user-facing commands that call the MCP tools:
