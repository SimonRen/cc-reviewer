 ...
 */
export function getReviewOutputJsonSchema(): object {
  return {
    type: 'object',
...
function normalizeReviewOutput(parsed: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!normalized.risk_assessment) {
    const ra = normalized.risk_assessment as undefined;
    ...
    const ra = normalized.risk_assessment as Record<string, unknown>;
 ...
 */
export function parseLegacyMarkdownOutput(markdown: string, reviewer: string): ReviewOutput | null {
  try {
    ...
    if (riskMatch) {
      ...
      if (levelMatch) {
        output.risk_assessment.overall_level = levelMatch[1].toLowerCase() as RiskAssessment['overall_level'];
