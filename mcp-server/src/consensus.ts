 ...
 */
export function groupSimilarFindings(
  reviews: Map<string, ReviewOutput>,
  similarityThreshold = 0.6
): Map<string, { finding: ReviewFinding; sources: string[] }[]> {
  const groups = new Map<string, { finding: ReviewFinding; sources: string[] }[]>();

  // Collect all findings with their source
  const allFindings: { finding: ReviewFinding; source: string }[] = [];
  ...
  const byCategory = new Map<string, typeof allFindings>();
  ...
  for (const [category, findings] of byCategory) {
    const clusters: { finding: ReviewFinding; sources: string[] }[] = [];
...
export function calculateConsensusScore(
  cluster: { finding: ReviewFinding; sources: string[] },
  totalModels: number,
 ...
 */
export function buildConsensusFindings(
  reviews: Map<string, ReviewOutput>,
  config: ConsensusConfig = DEFAULT_CONSENSUS_CONFIG
): ConsensusFinding[] {
 ...
 */
export function findUnanimousAgreements(reviews: Map<string, ReviewOutput>): string[] {
  if (reviews.size < 2) return [];

  const allAgreements = new Map<string, number>();
 ...
 */
export function detectConflicts(reviews: Map<string, ReviewOutput>): ModelConflict[] {
  const conflicts: ModelConflict[] = [];

  // Check for direct disagreements about the same claim
  const claimPositions = new Map<string, Record<string, 'agree' | 'disagree'>>();
    ...
    if (agrees.length > 0 && disagrees.length > 0) {
      conflicts.push({
        ...
        positions: Object.fromEntries(
          modelIds.map(m => [m, positions[m] === 'agree' ? 'Supports this claim' : 'Disputes this claim'])
        ),
      });
    }
  }
 ...
 */
export function findUniqueInsights(reviews: Map<string, ReviewOutput>): Record<string, string[]> {
  const groups = groupSimilarFindings(reviews);
 ...
 */
export function combineRiskAssessments(reviews: Map<string, ReviewOutput>): RiskAssessment {
 ...
 */
export function synthesizeCouncilReview(
  reviews: Map<string, ReviewOutput>,
  config: ConsensusConfig = DEFAULT_CONSENSUS_CONFIG
): CouncilReviewOutput {
 ...
 */
export function formatConsensusFindings(findings: ConsensusFinding[]): string {
  ...
  // Group by severity
  const bySeverity = new Map<string, ConsensusFinding[]>();
 ...
 */
export function formatConflicts(conflicts: ModelConflict[]): string {
 ...
 */
export function formatCouncilReview(review: CouncilReviewOutput): string {
  const lines: string[] = [];
