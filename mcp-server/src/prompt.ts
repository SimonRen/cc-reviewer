...
import { FocusArea, OutputType, FOCUS_AREA_DESCRIPTIONS } from './types.js';
import { getReviewOutputJsonSchema } from './schema.js';
import { ReviewRequest, ExpertRole, selectExpertRole, EXPERT_ROLES } from './adapters/base.js';
 ...
 */
function buildExpertSection(role: ExpertRole): string {
 ...
 */
export function buildReviewPrompt(options: PromptBuildOptions): string {
  const {
    request,
    expertRole,
    reviewerName,
...
export function buildPeerReviewPrompt(
  reviewerName: string,
  anonymizedReviewerId: string,
  reviewToScore: string,
  originalCcOutput: string
): string {
 ...
 */
export function buildDeveloperInstructions(cli: 'codex' | 'gemini'): string {
  const roleMap = {
 ...
 */
export function isValidFeedbackOutput(output: string): boolean {
  // Try JSON first
  try {
 ...
 */
export function detectOutputType(ccOutput: string): OutputType {
