...
import { FeedbackRequest, CliType, REVIEWER_PERSONAS, FOCUS_AREA_DESCRIPTIONS } from './types.js';

/**
 * Build the 7-section delegation prompt for external CLIs
 */
export function build7SectionPrompt(request: FeedbackRequest): string {
  const {
    workingDir,
    ccOutput,
    outputType,
 ...
 */
export function buildDeveloperInstructions(cli: CliType): string {
 ...
 */
export function detectOutputType(ccOutput: string): 'findings' | 'plan' | 'proposal' | 'analysis' {
